...
export const buscarRedPrimariaPorId = async (token, idPaciente) => {
    try {
        const result = await axiosInstance.get(`/paciente/red/${idPaciente}`, {
            headers: { Authorization: token },
        })
        return result.data
    } catch (error) {
        console.error(
            'Error al buscar la red primaria del paciente:',
            error.response?.data || error.message,
        )
        throw error
    }
...
export const actualizarRedPrimaraPaciente = async (token, formData) => {
    try {
        // Si solo tiene idPaciente e idRedPrimaria, es una asignación
        if (formData.idPaciente && formData.idRedPrimaria && Object.keys(formData).length === 2) {
            const data = {
            ...
            // Si tiene más datos, es una actualización de datos de la red primaria
            const data = {
                idPaciente: formData.idPaciente,
                idRedPrimaria: formData.idRedPrimaria,
                fecha: formData.fecha,

        ...
        <div className="grid grid-cols-5 md:grid-cols-5 gap-4">
          ...
          <div className="col-span-6">
            <Card className="w-full">
              <h5 className="text-lg font-semibold mb-4">Datos del Paciente</h5>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <Input disabled size="sm" value={item.data.identificacion} />
                </div>
                <div>
                  <label className="block text-sm font-bold mb-1">
                    Nacimiento
              ...
              <div className="flex mt-4 gap-x-1">
                ...
                <Button
                  ...
                  variant="solid"
                  title=""
                  onClick={() => handleClickSeguimiento(item.data.id)}
                />
              </div>
              ...
              >
                <div className="flex flex-col h-full space-y-4">
                  ...
                  <Section title="Datos de Ubicación">
                    ...
                    />
                    <Field label="Municipio" value={item.data.municipio} />
                    <Field
                      label="Dirección"
                      value={item.data.direccion}
                      span={2}
                    />
                    <Field label="Residente" value={item.data.residente} />
                    <Field label="Procedente" value={item.data.procedente} />
                  </Section>

                  {/* Información Socioeconómica */}
                  <Section title="Información Socioeconómica">
                    <Field label="Estrato" value={item.data.estrato} />
                    <Field label="Ocupación" value={item.data.ocupacion} />
                    <Field label="Régimen" value={item.data.regimen} />
                  </Section>

                  {/* Datos de Contacto */}
                  <Section title="Datos de Contacto">
                    <Field label="Celular" value={item.data.celular} />
                    <Field label="Correo" value={item.data.correo} span={2} />
                  </Section>
                </div>
              </Dialog>
                  ...
                  <div className="overflow-x-auto">
                    {loadingHistorias ? (
                      ...
                      <div className="text-center py-4 text-gray-500">
                        No hay historias clínicas disponibles para este
                        paciente.
                      </div>
                    ) : (
                      <Table>
                        <Table.THead>
                          <Table.Tr>
                            ...
                            <Table.Th>Usuario Finalizador</Table.Th>
                        ...
                        <Table.TBody>
                          {historiasClinicas.map((historia) => (
                            <Table.Tr key={historia.id}>
                              <Table.Td>
                                {historia.usuario_creador?.nombres &&
                                historia.usuario_creador?.apellidos
                              ...
                              <Table.Td>
                                {historia.usuario_finalizador?.nombres &&
                                historia.usuario_finalizador?.apellidos
                              ...
                              <Table.Td>
                                <Badge
                                  ...
                                  content={
                                    historia.estado ? "Abierta" : "Cerrada"
                                  }
                                />
                              </Table.Td>
                              <Table.Td>
                                <Button
                                  ...
                                  variant="solid"
                                  onClick={() => handleGeneratePDF(historia.id)}
                                ></Button>
                              </Table.Td>
                            </Table.Tr>
                          ))}
            ...
            <Card className="w-full ">
              <Tabs defaultValue="redPrimaria">
                <TabList className="flex flex-wrap space-x-4 border-b pb-2 overflow-hidden">
                  <TabNav value="redPrimaria" className="cursor-pointer">
                    Primaria
                  </TabNav>
                  <TabNav value="acompañante" className="cursor-pointer">
                    Acompañante
                  </TabNav>
                  <TabNav value="antFamiliar" className="cursor-pointer">
                    Familiar
                  </TabNav>
                  <TabNav value="antPerinatologicos" className="cursor-pointer">
                    Perinatológicos
                  </TabNav>
                  <TabNav value="vacunas" className="cursor-pointer">
                    Vacunas
                  </TabNav>
                  <TabNav value="primeraConsulta" className="cursor-pointer">
                    Ingreso
                  </TabNav>
                </TabList>
                <div className="p-4">
                  <TabContent value="redPrimaria">
                    {item.data.redPrimaria ? (
                      <InfoRedPrimaria data={item.data.redPrimaria} />
                    ) : (
                      <div className="text-gray-500 italic">
                        No hay información de red primaria registrada.
                      </div>
                    )}
                  </TabContent>
                  <TabContent value="acompañante">
                    {item.data.acompaniante ? (
                      <InfoAcompañante data={item.data.acompaniante} />
                    ) : (
                      <div className="text-gray-500 italic">
                        No hay información de acompañante registrada.
                      </div>
                    )}
                  </TabContent>

...
import { BuscarAcompañante, actualizarAcompañante } from '@/customService/services/acompañanteService'
...
interface EditFormAcompañanteProps {
    idPaciente: number
    acompañanteData?: any
    onClose?: () => void
}

function EditFormAcompañante({
    idPaciente,
    acompañanteData,
    onClose,
}: EditFormAcompañanteProps) {
    const {
        control,
        handleSubmit,
        formState: { errors },
    ...
    // Cargar datos del acompañante al montar el componente
    useEffect(() => {
        if (acompañanteData) {
            setSelectedDepartment(acompañanteData.departamento)
            reset({
                nombre: acompañanteData.nombre || '',
                apellido: acompañanteData.apellido || '',
                tipo_identificacion: acompañanteData.tipo_identificacion || '',
                identificacion: acompañanteData.identificacion || '',
                celular: acompañanteData.celular || '',
                correo: acompañanteData.correo || '',
                ocupacion: acompañanteData.ocupacion || '',
                municipio: acompañanteData.municipio || '',
                departamento: acompañanteData.departamento || '',
                direccion: acompañanteData.direccion || '',
                tipo_vivienda: acompañanteData.tipo_vivienda || '',
                nivel_ingreso: acompañanteData.nivel_ingreso || '',
                nivel_academico: acompañanteData.nivel_academico || '',
                tipo_vehiculo: acompañanteData.tipo_vehiculo || '',
            })
        }
    }, [acompañanteData, reset])

    const onSubmit = async (data) => {
        try {
            setLoading(true)

            if (!acompañanteData?.id) {
                console.error('No se encontró el ID del acompañante')
                return
            }

            // Preparar los datos para el servicio existente
            const formDataToSend = {
                idPaciente: idPaciente,
                idAcompanante: acompañanteData.id,
                ...data,
                id_user_update: user.id
            }

            const response = await actualizarAcompañante(token, formDataToSend)
    ...
    return (
        <div className="max-h-[60vh] overflow-y-auto p-4">
            ...
            >
                ...
                <SectionTitle
                    text="Información Básica del Acompañante"
                ...
                <InputForm
                    ...
                    errors={errors}
                    label="Apellido"
                    inputPlaceholder="Apellido del acompañante"
                    className="col-span-1"
                    value=""
                />
                <SelectDocumentType
...
export default EditFormAcompañante
